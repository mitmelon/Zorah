...
class Image
{
    ...
    {
        return (new \Manomite\Engine\Security\PostFilter)->sanitize_file_name($this->name);
    }
    ...
    public function is_valid_base64_image(string $image_string){
        if(preg_match('/data:image\/(png|jpg|jpeg);base64,(.*)/', $image_string, $matches)){
            $base64_string = $matches[2];
            $image_data = base64_decode($base64_string);
            $image_info = \getimagesizefromstring($image_data);
            return $image_info && in_array($image_info['mime'], ['image/png', 'image/jpg', 'image/jpeg']);
        } else {
            return false;
        }
    }
